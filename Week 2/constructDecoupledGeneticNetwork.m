...
for i=1:numPeople
	...
	if(parents(1)==0)%root node
		factorList(idx)=childCopyGivenFreqsFactor(alleleFreqs,i);
		idx+=1;
		factorList(idx)=childCopyGivenFreqsFactor(alleleFreqs,i+numPeople);
		idx+=1;
	else %internal node
		factorList(idx)=childCopyGivenParentalsFactor(numAlleles,i,parents(1),parents(1)+numPeople);
		idx+=1;
		factorList(idx)=childCopyGivenParentalsFactor(numAlleles,i+numPeople,parents(2),parents(2)+numPeople);
		idx+=1;
	end
	factorList(idx)=phenotypeGivenCopiesFactor(alphaList, numAlleles, i, i+numPeople, i+2*numPeople);
	idx+=1;
end
