...
for i=1:M%parent two
	A=genotypesToAlleles(i,:);
	for j=1:M%parent one
		C=genotypesToAlleles(j,:);
		for k=1:M%child
			E=genotypesToAlleles(k,:);
			c=0;
			if((E(1)==C(1) && E(2)==A(1)) || (E(2)==C(1) && E(1)==A(1)))
				c+=1;
			end
			if((E(1)==C(1) && E(2)==A(2)) || (E(2)==C(1) && E(1)==A(2)))
				c+=1;
			end
			if((E(1)==C(2) && E(2)==A(1)) || (E(2)==C(2) && E(1)==A(1)))
				c+=1;
			end
			if((E(1)==C(2) && E(2)==A(2)) || (E(2)==C(2) && E(1)==A(2)))
				c+=1;
			end
			idx=AssignmentToIndex([k,j,i],genotypeFactor.card);
			genotypeFactor.val(idx)=(c)/4;
		end
	end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%